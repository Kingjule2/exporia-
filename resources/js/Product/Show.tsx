...
function Show({ product, variationOptions }: {
    ...
    const computedProduct = useMemo(() => {
        ...
        for (let variation of product.variations) {
            ...
            if (arraysAreEqual(selectedOptionsIds, optionIds)) {
                return {
                    ...
                    quantity: variation.quantity === null ? Number.MAX_VALUE
                        : variation.quantity,
                };
            }
        }
        return {
            price: product.price,
            quantity: product.quantity === null ? Number.MAX_VALUE
                : product.quantity,
        };
    }, [product, selectedOptions]);
                        ...
                        <div className="flex gap-2 mb-4">
                            {type.options.map(option => (
                                <div onClick={() => chooseOption(type.id, option)} key={option.id}>
                                    {option.images.length > 0 && <img src={option.images[0].thumb} alt="" className={'w-[50px] ' + (
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="container mx-auto p-8">
                <div className="grid gap-8 grid-color-1 lg:grid-cols-12">
